"""Run the Wan 2.2 first-to-last-frame video workflow via the SDK.

This example mirrors the following request shape:
    POST /openapi/v2/run/workflow/2011275998205054977
    {
      "addMetadata": true,
      "nodeInfoList": [],
...
If you provide local first/last frame paths, the script uploads them and injects
them into the workflow's ImageLoader nodes before submission.
...
DEFAULT_FIRST_FRAME_NODE_ID = "43"
DEFAULT_LAST_FRAME_NODE_ID = "44"
DEFAULT_TEXT_NODE_ID = "30"
DEFAULT_HIGH_NOISE_SAMPLER_NODE_ID = "27"
DEFAULT_LOW_NOISE_SAMPLER_NODE_ID = "28"
...
def build_payload(client: RunningHubClient) -> Dict[str, Any]:
    ...
    first_frame_path = get_optional_env("RUNNINGHUB_FIRST2LAST_FIRST_FRAME_PATH")
    last_frame_path = get_optional_env("RUNNINGHUB_FIRST2LAST_LAST_FRAME_PATH")
    if bool(first_frame_path) ^ bool(last_frame_path):
        raise SystemExit(
            "RUNNINGHUB_FIRST2LAST_FIRST_FRAME_PATH and "
            "RUNNINGHUB_FIRST2LAST_LAST_FRAME_PATH must be set together."
        )

    if first_frame_path and last_frame_path:
        first_frame_node_id = (
            get_optional_env("RUNNINGHUB_FIRST2LAST_FIRST_FRAME_NODE_ID")
            or DEFAULT_FIRST_FRAME_NODE_ID
        )
        last_frame_node_id = (
            get_optional_env("RUNNINGHUB_FIRST2LAST_LAST_FRAME_NODE_ID")
    ...
    text_node_id = get_optional_env("RUNNINGHUB_FIRST2LAST_TEXT_NODE_ID") or DEFAULT_TEXT_NODE_ID
    positive_prompt = get_optional_env("RUNNINGHUB_FIRST2LAST_POSITIVE_PROMPT")
    negative_prompt = get_optional_env("RUNNINGHUB_FIRST2LAST_NEGATIVE_PROMPT")
    if positive_prompt:
        modifier.set(text_node_id, "positive_prompt", positive_prompt)
    if negative_prompt:
        modifier.set(text_node_id, "negative_prompt", negative_prompt)

    shared_seed = get_optional_int("RUNNINGHUB_FIRST2LAST_SEED")
    high_noise_seed = get_optional_int("RUNNINGHUB_FIRST2LAST_HIGH_NOISE_SEED")
    low_noise_seed = get_optional_int("RUNNINGHUB_FIRST2LAST_LOW_NOISE_SEED")
    ...
    high_noise_sampler_node_id = (
        get_optional_env("RUNNINGHUB_FIRST2LAST_HIGH_NOISE_SAMPLER_NODE_ID")
        or DEFAULT_HIGH_NOISE_SAMPLER_NODE_ID
    )
    low_noise_sampler_node_id = (
        get_optional_env("RUNNINGHUB_FIRST2LAST_LOW_NOISE_SAMPLER_NODE_ID")
        or DEFAULT_LOW_NOISE_SAMPLER_NODE_ID
    )

    if high_noise_seed is not None:
        modifier.seed(high_noise_sampler_node_id, high_noise_seed)
    if low_noise_seed is not None:
        modifier.seed(low_noise_sampler_node_id, low_noise_seed)
...
def wait_for_result(client: RunningHubClient, task_id: str) -> None:
    poll_interval = float(os.getenv("RUNNINGHUB_FIRST2LAST_POLL_INTERVAL", "5"))
    timeout = float(os.getenv("RUNNINGHUB_FIRST2LAST_TIMEOUT", "1800"))
    ...
    download_dir = REPO_ROOT / "downloads" / "wan22_first2last_video" / task_id
...
def main() -> int:
    ...
    workflow_id = os.getenv("RUNNINGHUB_FIRST2LAST_WORKFLOW_ID", DEFAULT_WORKFLOW_ID).strip()
    ...
    try:
        ...
        print(f"Wan 2.2 first-to-last video workflow task failed: {exc}", file=sys.stderr)
        return 1

    print_section("Done")
    print("Wan 2.2 first-to-last video workflow finished successfully.")
    return 0
